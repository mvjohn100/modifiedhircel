class CompaniesController < ApplicationController
  ...
  def employees
    ...
    if params[:name].nil?
      ...
      users=User.includes(:resources, :tasks ,:profile).where(" users.department_id is NULL and users.company_id = ? and ( users.first_name like ? or users.last_name like ? )",params[:id],"%#{params[:name]}%","%#{params[:name]}%")
    end
  ...
  def departments
     ...
     manager = User.find(:first, 
        ...
        :conditions => ["users.company_id=? and roles.name='manager'",params[:id]]
      ) 
